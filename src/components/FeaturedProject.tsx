    ...
    return (
        <div className={"grid grid-cols-5 my-24" + (props.reverse ? " lg:text-right" : "")}>
            <div
                ...
                <h3 className={"w-max font-bold text-2xl mt-2 pointer-events-auto" + (props.reverse ? " lg:self-end" : "")}>{props.title}</h3>
            ...
            <motion.div whileHover={{ scale: 1.05 }}
                ...
                <a className={"h-full w-full bg-emerald-400 rounded cursor-pointer opacity-0 hover:opacity-50 absolute top-0 left-0"}></a>
            </motion.div>
        </div>
    )
}